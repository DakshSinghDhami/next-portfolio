import { PROJECTS, NEW_PROJECTS, OSS_PROJECTS } from "@/lib/data";
...
function CompactProjectCard({ project }: { project: (typeof NEW_PROJECTS)[number] }) {
  return (
    <div className="group relative rounded-lg border border-base-800/60 bg-base-900/30 p-3.5 transition-all duration-300 hover:border-accent-cyan/40 hover:bg-base-900/60">
      {/* Project Number */}
      <div className="text-2xl font-bold text-base-800 select-none mb-2 transition-colors group-hover:text-accent-cyan/20">
        {String(project.id).padStart(2, "0")}
      </div>

      {/* Title */}
      <h3 className="text-sm font-semibold text-white mb-1.5 group-hover:text-accent-cyan transition-colors">
        {project.title}
      </h3>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex flex-wrap gap-1.5">
          ...
          {project.tags.length > 2 && (
            <span className="inline-flex items-center rounded-md bg-base-800/60 px-2 py-0.5 text-[10px] font-medium text-base-600 border border-base-700/50">
              +{project.tags.length - 2}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1 text-[10px] font-medium text-accent-cyan hover:text-accent-cyan/80 transition-colors"
            title="Launch"
          >
            <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1 text-[10px] font-medium text-base-500 hover:text-white transition-colors"
            title="Source"
          >
      ...
      <section className="relative pb-24 sm:pb-32">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-12">
            ...
            <div className="h-1 w-20 bg-gradient-to-r from-accent-cyan to-accent-violet rounded-full mx-auto mb-4" />
            <p className="text-base-400 max-w-2xl mx-auto">
              New interactive canvas experiments and tools recently added to the
              collection.
            </p>
          </div>
